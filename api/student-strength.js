...
module.exports = async (req, res) => {
  ...
  try {
    const url = `${ERP_BASE}/getSISDashboard/dashboard?entity=${encodeURIComponent(entity)}&session=${encodeURIComponent(session)}`;
    ...
    const headCount = data.headCount?.currentSession?.[0] || {};
    const groupByCourse = data.groupByCourse || [];

    res.status(200).json({
      entity: { id: match.id, name: match.name },
      session,
      headCount,
      groupByCourse,
    });
