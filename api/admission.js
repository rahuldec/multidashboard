...
module.exports = async (req, res) => {
  ...
  try {
    const url = `${ERP_BASE}/getAdmissionDashboard/dashboard?entity=${encodeURIComponent(entity)}&session=${encodeURIComponent(session)}&admissionSession=${encodeURIComponent(session)}`;
    ...
    res.status(200).json({ entity: { id: match.id, name: match.name }, session, ...data });
  } catch (err) {
